...
export function headerScroll() {
	...
	$(window).scroll(() => { // при прокрутке страницы
		...
		const headerHidden = () => header.hasClass('_header-hidden') // узнаем скрыт header или нет
		if (currentScroll > prevScroll && !headerHidden()) { // если прокручиваем страницу вниз и header не скрыт
			
			if(currentScroll > 200){
				header.addClass('_header-hidden') // то скрываем header
			}
		}
		if (currentScroll < prevScroll && headerHidden()) { // если прокручиваем страницу вверх и header скрыт
			header.removeClass('_header-hidden') // то отображаем header
		}
